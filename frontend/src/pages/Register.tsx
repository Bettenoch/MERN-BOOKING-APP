...
const Register = () => {
    ...
    const mutation = useMutation(apiClient.register, {
        onSuccess: () => {
           showToast({message: 'Registartion sucessful', type: 'SUCCESS'});
           navigate('/');
        },
        onError: (error: Error) => {
  ...
  return (
    <form action="" className=" px-16 flex flex-col gap-5" onSubmit={onSubmit}>
        ...
        <label className="text-gray-700 text-sm font-bold flex-1">
            Password
            <input type="confirmPassword"  className="border rounded w-full py-1 px-2 font-normal" {...register('confirmPassword', 
            {
                validate: (val) => {
                    if(!val) {
        ...
        <span>
            <button type="submit" className="bg-blue-600 text-xl text-white p-2 font-bold hover:bg-blue-500">
                Sign Up
            </button>
        </span>
    </form>
